...
NE_plot<-ggplot(acs)+geom_sf(data=sts_NE, color="black")+
  ...
  geom_point(data = NE_dat,
             aes(x = Longitude, y = Latitude, colour = Arithmetic.Mean),
             size = 1.25)+
  scale_color_gradient(low = "blue", high = "red", name = 'PM2.5')+
  labs(x=NULL, y=NULL) +
...
#plot data 3
prep1%>%
  mutate(AAMR_ICDI= cut(AAMR_ICDI_aa, breaks=quantile(AAMR_ICDI_aa, p=seq(0,1,length.out = 5), na.rm=T ), include.lowest=T ))%>%
...
# 2/11/23 not using for now... a good chunk is marked as unreliable
# #plot data 4
# prep1%>%
#   mutate(AAMR_ICDI_65= cut(AAMR_ICDI_65, breaks=quantile(AAMR_ICDI_65, p=seq(0,1,length.out = 5), na.rm=T ), include.lowest=T ))%>%
...
```{r}
#load NLCD download
NLCD<-raster('C:/Users/tamas/OneDrive/Documents/APDEM 808/data files/nlcd_2019_land_cover_l48_20210604.img')
...
#add leading zeroes
NLCD_vec_c <- NLCD_15%>% pull(cofips)
...
W_box1<-subset(c_prep4,select=c(1:2,32:48))%>%
...
#reshape for box plots
SE_box1<-subset(c_prep4,select=c(1:2,32:48))%>%
...
#reshape for box plots
OV_box1<-subset(c_prep4,select=c(1:2,32:48))%>%
...
#reshape for box plots
NW_box1<-subset(c_prep4,select=c(1:2,32:48))%>%
...
```{r}
#prep data
smokers_c <- read_excel('C:/Users/tamas/OneDrive/Documents/APDEM 808/data files/2016 smoking-v3.xlsx')

c_prep3<-left_join(c_prep2,smokers_c, by="cofips")
c_prep3<-subset(c_prep3,select=-c(32,33))
