...
c_prep7<-subset(c_prep7,select=-c(75:77)) 
...
c_df2_a <-subset(c_df1,select=-c(2:5,7,9:11,13,15:29,34,38:40))%>%
    filter(complete.cases(AAMR_all_ICDI_use,pct_white,pct_black,pct_hispanic,pct_bach,pct_pov,PM2.5,pct_smokers,pct_obese,pct_uninsured,entropy,TMAX,PCP))

 #removing items not needed in model B & filter complete cases                              
c_df2_b <-subset(c_df1,select=-c(2:6,9:11,13,15:29,34,38:40))%>%
    filter(complete.cases(AAMR_all_ICDI65_use,pct_white,pct_black,pct_hispanic,pct_bach,pct_pov,PM2.5,pct_smokers,pct_obese,pct_uninsured,entropy,TMAX,PCP))
...
#OLS model A
fit_c_df_a<- lm(AAMR_all_ICDI_use ~ pct_white + pct_black + pct_hispanic + pct_bach + pct_pov + PM2.5 + pct_smokers + pct_obese + pct_uninsured + entropy + TMAX + PCP, data = c_df6_a)

# #OLS model B
fit_c_df_b<- lm(AAMR_all_ICDI65_use ~ pct_white + pct_black + pct_hispanic + pct_bach + pct_pov + PM2.5 + pct_smokers + pct_obese + pct_uninsured + entropy + TMAX + PCP, data = c_df6_b)
...
fit_df2_a_err <- errorsarlm(AAMR_all_ICDI_use ~ pct_white + pct_black + pct_hispanic + pct_bach + pct_pov + PM2.5 + pct_smokers + pct_obese + pct_uninsured + entropy + TMAX + PCP, data = c_df6_a, listw = df_1a_wt6)

fit_df2_b_err <- errorsarlm(AAMR_all_ICDI65_use ~ pct_white + pct_black + pct_hispanic + pct_bach + pct_pov + PM2.5 + pct_smokers + pct_obese + pct_uninsured + entropy + TMAX + PCP, data = c_df6_b, listw = df_1b_wt6)
