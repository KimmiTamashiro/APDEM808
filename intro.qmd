...
US_box1<-subset(c_prep5,select=c(1:2,48,65:73)) #entropy all major categories
US_box2<-subset(c_prep5,select=c(1:2,48,51:54,73)) #entropy focused on developed types
...
#careful of reshape2 warning
US_box2<-reshape2::melt(US_box2,id.vars=c('cofips','NAME','reg_names'),measure.vars=c('dev_open','dev_low','dev_med','dev_high','all_else'))
...
entropy_dev$entropy_dev <- as.numeric((entropy_dev$entropy_dev_prep *-1) / log(5))
...
c_prep6%>%
  ...
  scale_y_continuous(label=scales::comma)+
  geom_sf(data=sts, color="black")+
  ggtitle("Land Use Entropy Index by County- Developed Focus", subtitle = "NLCD 2011")
...
#adding note so i can push to git
